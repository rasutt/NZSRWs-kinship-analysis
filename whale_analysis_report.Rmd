...
This is a report I was making to separate various outputs that I thought were worth reporting from the code to process the data and save the results.  I never got it to even a first draft, and it's out of sync with the rest of the code now, for example there's no whale_analysis.Rdata anymore, but seems like its probably worth tidying up.  Might be a nice way to summarise the work.

## Microsatellite genotypes, missing data, and homozygosity

```{r}
# Load imported and processed data.
load("whale_analysis.Rdata")


# Review missing data

# Show rows where one allele missing but other not
whale_data[rowSums(missing_ales_1 != missing_ales_2) > 0, ]

# Three of these are rows with blank cells that happen to start halfway through
# a locus. I should check whether this data exists in the larger database.  

# One is the line that had the OOB entry.

# Show proportion of missing data overall
round(mean(missing_ales_1), 3)

# Find and show proportions of missing data by locus from largest to smallest
props_missing <- round(colMeans(missing_ales_1), 3)
names(props_missing) <- locus_names
props_missing[order(prop_missing, decreasing = T)]
...
round(
  ...
  )[head(order(abs(props_homo_unexp), decreasing = T)), ],
  3
)
